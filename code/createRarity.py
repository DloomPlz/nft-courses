# For each folder in assets, create a json file : data.json
# The json needs to contain all images that is in the same folder 

#{
#  "trait_type": "<FOLDER_NAME>",
#  "attributes": [
#    {
#      "name": "<IMAGE_NAME_WIHTOUT_EXTENSION_AND_SEMANTIC>",
#      "filename": "<FOLDER_NAME>/<FILENAME_PATH>",
#      "rarity": 
#    },
#    ...
#  ]
    ...
    if os.path.isdir(d):
        ...
        for filename in os.listdir(d):
            ...
            if os.path.isfile(f):
                ...
                if(extension == ".png"):
                    name = os.path.basename(os.path.normpath(f).replace(".png",""))
                    filenamePath = f.replace("assets/","").replace("assets\\","")
        ...
        rest = 0
        if emptyRarity == 0:
            rest = 100 - ((rarityMedian * (len(baseJson["attributes"])) + emptyRarity ))
        else : rest = 100 - ((rarityMedian * (len(baseJson["attributes"]) - 1)) + emptyRarity )
